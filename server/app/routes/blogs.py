...
from app.models import BlogLink, _generate_id
...
def create_blog_link(data: BlogLinkCreate, db: Session = Depends(get_db)):
    link = BlogLink(
        id=_generate_id(),
        title=data.title,
        url=data.url,
...
def update_blog_link(
    blog_id: str, data: BlogLinkUpdate, db: Session = Depends(get_db)
):
...
def delete_blog_link(blog_id: str, db: Session = Depends(get_db)):
    link = db.query(BlogLink).filter(BlogLink.id == blog_id).first()
    if not link:
        raise HTTPException(status_code=404, detail="Blog link not found")
    db.delete(link)
